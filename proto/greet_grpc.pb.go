...
type GreetServiceClient interface {
	...
	SayHelloBidirectinalStreaming(ctx context.Context, opts ...grpc.CallOption) (GreetService_SayHelloBidirectinalStreamingClient, error)
}
...
func (c *greetServiceClient) SayHelloBidirectinalStreaming(ctx context.Context, opts ...grpc.CallOption) (GreetService_SayHelloBidirectinalStreamingClient, error) {
	stream, err := c.cc.NewStream(ctx, &GreetService_ServiceDesc.Streams[2], "/greet_service.GreetService/SayHelloBidirectinalStreaming", opts...)
	if err != nil {
		return nil, err
	}
	x := &greetServiceSayHelloBidirectinalStreamingClient{stream}
	return x, nil
}

type GreetService_SayHelloBidirectinalStreamingClient interface {
	Send(*HelloRequest) error
	Recv() (*HelloResponse, error)
	grpc.ClientStream
}

type greetServiceSayHelloBidirectinalStreamingClient struct {
	grpc.ClientStream
}

func (x *greetServiceSayHelloBidirectinalStreamingClient) Send(m *HelloRequest) error {
	return x.ClientStream.SendMsg(m)
}

func (x *greetServiceSayHelloBidirectinalStreamingClient) Recv() (*HelloResponse, error) {
	m := new(HelloResponse)
	if err := x.ClientStream.RecvMsg(m); err != nil {
		return nil, err
	}
...
type GreetServiceServer interface {
	...
	SayHelloBidirectinalStreaming(GreetService_SayHelloBidirectinalStreamingServer) error
...
func (UnimplementedGreetServiceServer) SayHelloBidirectinalStreaming(GreetService_SayHelloBidirectinalStreamingServer) error {
	return status.Errorf(codes.Unimplemented, "method SayHelloBidirectinalStreaming not implemented")
}
...
func _GreetService_SayHelloBidirectinalStreaming_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(GreetServiceServer).SayHelloBidirectinalStreaming(&greetServiceSayHelloBidirectinalStreamingServer{stream})
}

type GreetService_SayHelloBidirectinalStreamingServer interface {
	Send(*HelloResponse) error
	Recv() (*HelloRequest, error)
	grpc.ServerStream
}

type greetServiceSayHelloBidirectinalStreamingServer struct {
	grpc.ServerStream
}

func (x *greetServiceSayHelloBidirectinalStreamingServer) Send(m *HelloResponse) error {
	return x.ServerStream.SendMsg(m)
}

func (x *greetServiceSayHelloBidirectinalStreamingServer) Recv() (*HelloRequest, error) {
	m := new(HelloRequest)
	if err := x.ServerStream.RecvMsg(m); err != nil {
		return nil, err
	}
...
var GreetService_ServiceDesc = grpc.ServiceDesc{
	...
	Streams: []grpc.StreamDesc{
		...
		{
			StreamName:    "SayHelloBidirectinalStreaming",
			Handler:       _GreetService_SayHelloBidirectinalStreaming_Handler,
			ServerStreams: true,
			ClientStreams: true,
		},
	},
